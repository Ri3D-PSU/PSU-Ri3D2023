...
/**
 * A command that uses a RAMSETE controller ({@link RamseteController}) to follow a trajectory {@link Trajectory} with a
 * differential drive.
 *
 * <p>The command handles trajectory-following, PID calculations, and feedforwards internally. This
 * is intended to be a more-or-less "complete solution" that can be used by teams without a great deal of controls
 * expertise.
 *
 * <p>Advanced teams seeking more flexibility (for example, those who wish to use the onboard PID
 * functionality of a "smart" motor controller) may use the secondary constructor that omits the PID and feedforward
 * functionality, returning only the raw wheel speeds from the RAMSETE controller.
 *
...
public class RamseteCommand extends CommandBase {
    ...
    /**
     * Constructs a new RamseteCommand that, when executed, will follow the provided trajectory. PID control and
     * feedforward are handled internally, and outputs are scaled -12 to 12 representing units of volts.
     *
     ...
     * @param outputVolts     A function that consumes the computed left and right outputs (in volts) for the robot
     *                        drive.
    ...
    /**
     * Constructs a new RamseteCommand that, when executed, will follow the provided trajectory. Performs no PID control
     * and calculates no feedforwards; outputs are the raw wheel speeds from the RAMSETE controller, and will need to be
    ...
    public boolean isFinished() {
        return m_timer.hasElapsed(m_trajectory.getTotalTimeSeconds() + 3);
    }
